      ...
      (* Auth: token exchange *)
      {
        ...
        body =
          Yojson.Safe.to_string
            (`Assoc [ ("access_token", `String "ACCESS"); ("expires_in", `Int 600) ]);
      }
    else begin
      captured := Some req;
...
let test_bars_request_url_and_params () =
  ...
  Alcotest.(check string)
    "path" "/trade-api-market-data-connector/api/v1/candles-chart" path;
  let qp name =
...
let test_bars_decimal_decoded () =
  ...
  Alcotest.(check (float 1e-6)) "last close = 320.5" 320.5 (Decimal.to_float last.close);
  Alcotest.(check (float 1e-6)) "last volume" 1500.0 (Decimal.to_float last.volume)
...
let test_bare_ticker_uses_default_class_code () =
  ...
  let _ =
    Rest.bars rest ~n:10
      ~instrument:
        (Instrument.make ~ticker:(Ticker.of_string "AAPL") ~venue:(Mic.of_string "MISX")
           ())
      ~timeframe:H1
  in
  let req = !captured in
  Alcotest.(check string)
    "classCode defaults to config" "SPBXM"
    (Option.value (Uri.get_query_param req.url "classCode") ~default:"<missing>");
  Alcotest.(check string)
    "ticker pass-through" "AAPL"
    (Option.value (Uri.get_query_param req.url "ticker") ~default:"<missing>")
...
let test_bars_caps_n_at_max () =
  ...
  let start_str =
    Option.value (Uri.get_query_param !captured.url "startDate") ~default:""
  in
  let end_str = Option.value (Uri.get_query_param !captured.url "endDate") ~default:"" in
  Alcotest.(check bool)
    "startDate is ISO 8601" true
    (String.length start_str >= 20 && String.get start_str 10 = 'T');
  Alcotest.(check bool) "endDate is ISO 8601" true (String.length end_str >= 20)
...
let test_route_instrument_falls_back_to_default () =
  let cfg =
    Config.make ~default_class_code:"SPBXM"
      ~rest_base:(Uri.of_string "https://api.test")
      ~token_endpoint:(Uri.of_string "https://api.test/token")
      ()
  in
  let inst =
    Instrument.make ~ticker:(Ticker.of_string "FXUS") ~venue:(Mic.of_string "MISX") ()
  in
...
let tests =
  [
    ...
    ( "bare ticker uses default classCode",
      `Quick,
      test_bare_ticker_uses_default_class_code );
    ("caps n at 1440", `Quick, test_bars_caps_n_at_max);
    ("route_instrument uses board", `Quick, test_route_instrument_uses_board);
    ( "route_instrument default fallback",
      `Quick,
      test_route_instrument_falls_back_to_default );
  ]
