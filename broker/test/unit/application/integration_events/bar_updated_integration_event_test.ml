...
let test_of_domain_maps_all_fields () =
  ...
  Alcotest.(check string) "ts" "2023-11-14T22:13:20Z" dto.candle.ts;
  ...
  Alcotest.(check string) "open" "100.5" dto.candle.open_;
  Alcotest.(check string) "high" "101" dto.candle.high;
  Alcotest.(check string) "low" "100" dto.candle.low;
  Alcotest.(check string) "close" "100.75" dto.candle.close;
  Alcotest.(check string) "volume" "1234" dto.candle.volume
...
let test_yojson_roundtrip () =
  ...
  Alcotest.(check string) "ts" dto.candle.ts restored.candle.ts;
  Alcotest.(check string) "close" dto.candle.close restored.candle.close
