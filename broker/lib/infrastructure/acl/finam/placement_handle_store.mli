(** ACL-private mapping {[placement_id → client_order_id]} for
    ...
    Finam speaks its own venue identity ([clientOrderId], Finam's
    "letters / numbers / space" format) — a concept that has no place in our model.
    The cross-BC saga key [placement_id : int] is all the
    application layer carries; this store is the bridge between
    the two, scoped to the Finam adapter's lifetime.
