...
let execute_pending config state (c : Candle.t) : state * (Signal.t * settled) option =
...
let advance_strategy config state (c : Candle.t) : state =
  let strat', sig_ = Strategies.Strategy.on_candle state.strat config.instrument c in
  let pending = if sig_.Signal.action = Signal.Hold then None else Some sig_ in
  { state with strat = strat'; pending_signal = pending; last_bar_ts = c.ts }
