...
let test_fill_in_working_emits_commit_fill () =
  ...
  Alcotest.(check bool) "stays in Working" true
    (match s1 with Pm.Working _ -> true | _ -> false);
  Alcotest.(check int) "exactly one command" 1 (List.length cmds);
  Alcotest.(check bool) "command is Commit_fill" true
    (List.exists is_commit_fill cmds)
...
let test_ticket_completed_settles_with_no_command () =
  ...
  Alcotest.(check bool) "transitions to Settled" true
    (match s1 with Pm.Settled { reservation_id = 42 } -> true | _ -> false);
...
let test_ticket_cancelled_releases () =
  ...
  Alcotest.(check bool) "transitions to Released" true
    (match s1 with Pm.Released { reservation_id = 42; _ } -> true | _ -> false);
...
let tests =
  [
    Alcotest.test_case "Amount_reserved → Working + Dispatch_open_ticket"
      `Quick test_amount_reserved_advances_to_working;
