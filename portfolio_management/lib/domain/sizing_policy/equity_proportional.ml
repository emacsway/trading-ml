...
let size_scalar ~book_equity ~mark ~(book_id : Common.Book_id.t)
    ~(instrument : Instrument.t) ~direction ~strength ~source ~observed_at :
    Common.Target_proposal.t =
  let weight = scalar_weight ~direction ~strength in
  let target_qty =
    qty_of ~book_equity ~mark_price:(mark instrument) ~weight
  in
...
let size_coupled ~book_equity ~mark ~(book_id : Common.Book_id.t)
  ...
  let positions =
    List.map
      (fun (leg : Common.Construction_intent.leg) ->
        let target_qty =
          qty_of ~book_equity ~mark_price:(mark leg.instrument)
            ~weight:leg.weight
        in
        ({
           book_id;
...
let size () ~book_equity ~mark ~volatility:_ (intent : Common.Construction_intent.t)
  ...
  | Common.Construction_intent.Scalar s ->
      size_scalar ~book_equity ~mark ~book_id:s.book_id
  ...
  | Common.Construction_intent.Coupled c ->
      size_coupled ~book_equity ~mark ~book_id:c.book_id ~legs:c.legs
        ~coupling:c.coupling ~source:c.source ~observed_at:c.observed_at
