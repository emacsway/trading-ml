(** Command handler for {!Define_pair_mr_command.t}.

    Validates the wire shape into a {!Pair_mr_config.t}, builds
    the initial {!Pair_mr_state.t}, and hands the result to the
    [persist_pair_mr_state] closure for storage in the per-book
    pair-mr registry. *)
...
val handle :
  persist_pair_mr_state:
    (book_id:Portfolio_management.Common.Book_id.t ->
    pair:Portfolio_management.Common.Pair.t ->
    state:Portfolio_management.Pair_mean_reversion.state ->
    unit) ->
  Define_pair_mr_command.t ->
