...
let test_create_limit_order_payload () =
  ...
  Alcotest.(check string) "market type" "1"
    (Rest.bcs_order_type_of Market);
  Alcotest.(check string) "limit type" "2"
...
let test_decode_order_status () =
  let cfg = make_cfg () in
  let o = Rest.bcs_order_of_json cfg
    (Yojson.Safe.from_string sample_order_status) in
  Alcotest.(check string) "id"
    "3fa85f64-5717-4562-b3fc-2c963f66afa6" o.id;
  Alcotest.(check string) "exchangeId" "EX001" o.exec_id;
  Alcotest.(check string) "status" "NEW"
  ...
  Alcotest.(check (option string)) "board"
    (Some "TQBR") (Option.map Board.to_string (Instrument.board o.instrument));
  ...
  (match o.kind with
   | Limit p ->
     Alcotest.(check (float 1e-6)) "price" 150.50 (Decimal.to_float p)
...
let test_status_mapping () =
  ...
  List.iter (fun (wire, expected) ->
    ...
    Alcotest.(check string) ("status " ^ wire)
      (Order.status_to_string expected) (Order.status_to_string got)
  ) cases
