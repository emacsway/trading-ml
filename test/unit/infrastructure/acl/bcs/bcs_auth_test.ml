...
let test_initial_call_posts_form_encoded () =
  ...
  Alcotest.(check string) "content-type"
    "application/x-www-form-urlencoded"
    (List.assoc "Content-Type" req.headers);
  let body = Option.value req.body ~default:"" in
  Alcotest.(check bool) "body has grant_type=refresh_token" true
    (String.length body > 0
     && (try let _ = Str.search_forward
                       (Str.regexp "grant_type=refresh_token") body 0 in
             true
         with Not_found -> false));
  Alcotest.(check bool) "body has refresh_token=REFRESH" true
    (try let _ = Str.search_forward
                   (Str.regexp "refresh_token=REFRESH") body 0 in
         true
     with Not_found -> false);
...
let test_caches_until_expiry () =
  ...
  let _ = Auth.current auth in
  let _ = Auth.current auth in
  Alcotest.(check int) "single HTTP call across reads"
    1 (List.length !requests)

let test_invalidate_triggers_reauth () =
  let requests = ref [] in
  let tok = make_jwt
    ~exp_epoch:(int_of_float (Unix.gettimeofday ()) + 600) in
  let transport : Http_transport.t = fun req ->
    requests := req :: !requests;
    { status = 200; body = token_body ~access:tok ~expires_in:600 }
  in
  ...
  let _ = Auth.current auth in
  Alcotest.(check int) "invalidate forces a second POST"
...
let test_refresh_when_expiry_near () =
  ...
  let requests = ref [] in
  let tok1 = make_jwt
    ~exp_epoch:(int_of_float (Unix.gettimeofday ()) - 10) in
  let tok2 = make_jwt
    ~exp_epoch:(int_of_float (Unix.gettimeofday ()) + 600) in
  let transport : Http_transport.t = fun req ->
    requests := req :: !requests;
