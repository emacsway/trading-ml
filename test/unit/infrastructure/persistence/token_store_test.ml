...
let test_env_save_is_noop () =
  ...
  Alcotest.(check (option string)) "still None after save"
    None (Token_store.load s)
...
let tmp_path () =
  ...
  Filename.concat base
    (Printf.sprintf "token_store_test_%d_%d"
...
let test_file_save_and_load () =
  with_tmp (fun path ->
    ...
    Alcotest.(check (option string)) "round-trip"
      (Some "secret-value") (Token_store.load s))

let test_file_trims_trailing_newline () =
  with_tmp (fun path ->
    (* Simulate a human-edited file with a trailing newline. *)
    Out_channel.with_open_text path
      (fun oc -> Out_channel.output_string oc "from-shell\n");
    let s = Token_store.file ~path in
    Alcotest.(check (option string)) "trimmed"
      (Some "from-shell") (Token_store.load s))
...
let test_file_chmod_600 () =
  with_tmp (fun path ->
    ...
    Alcotest.(check int) "mode = 0o600"
      0o600 (stat.st_perm land 0o777))
...
let test_file_overwrite_is_atomic () =
  with_tmp (fun path ->
    ...
    Alcotest.(check (option string)) "newest wins"
      (Some "second") (Token_store.load s);
    (* No stale tmp left behind. *)
    Alcotest.(check bool) "tmp cleaned up"
...
let test_fallback_reads_primary_first () =
  ...
  Alcotest.(check (option string)) "primary wins"
    (Some "primary-val") (Token_store.load s)
...
let test_fallback_falls_through_when_primary_empty () =
  ...
  let s = Token_store.fallback primary secondary in
  Alcotest.(check (option string)) "falls back"
    (Some "bootstrap") (Token_store.load s)
...
let test_fallback_save_goes_to_primary_only () =
  ...
  Alcotest.(check (option string)) "primary now holds rotated"
    (Some "rotated") (Token_store.load primary);
  Alcotest.(check (option string)) "secondary untouched"
    (Some "seed") (Token_store.load secondary)
...
let tests = [
  ...
  "fallback through to secondary",    `Quick, test_fallback_falls_through_when_primary_empty;
