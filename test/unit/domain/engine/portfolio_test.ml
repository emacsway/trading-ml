...
let test_partial_close_realizes_pnl () =
  ...
  let p = Engine.Portfolio.fill p
    ~instrument:inst ~side:Buy ~quantity:(d 10.0)
    ~price:(d 100.0) ~fee:Decimal.zero
  in
  let p = Engine.Portfolio.fill p
    ~instrument:inst ~side:Sell ~quantity:(d 5.0)
...
let test_flip_from_long_to_short () =
  let p = Engine.Portfolio.empty ~cash:(d 10000.0) in
  let p = Engine.Portfolio.fill p
    ~instrument:inst ~side:Buy ~quantity:(d 5.0)
    ~price:(d 100.0) ~fee:Decimal.zero
  in
  let p = Engine.Portfolio.fill p
    ~instrument:inst ~side:Sell ~quantity:(d 8.0)
...
let test_reserve_buy_reduces_available_cash () =
  ...
  let p = Engine.Portfolio.reserve p
    ~id:1 ~side:Buy ~instrument:inst
    ~quantity:(d 10.0) ~price:(d 100.0)
    ~slippage_buffer:0.01   (* 1% — reserve qty*price*1.01 *)
    ~fee_rate:0.001         (* 0.1% fee estimate *)
  in
  Alcotest.(check bool) "cash unchanged" true
    (Decimal.compare p.cash (d 10_000.0) = 0);
  (* reserved = 10 * 100 * 1.01 = 1010, fee = 10 * 100 * 0.001 = 1,
     total 1011. Available = 10000 - 1011 = 8989. *)
  Alcotest.(check (float 1e-3)) "available cash"
    8989.0 (Decimal.to_float (Engine.Portfolio.available_cash p))

let test_reserve_sell_does_not_touch_cash () =
  let p = Engine.Portfolio.empty ~cash:(d 10_000.0) in
  let p = Engine.Portfolio.fill p ~instrument:inst ~side:Buy
    ~quantity:(d 20.0) ~price:(d 100.0) ~fee:Decimal.zero in
  let p = Engine.Portfolio.reserve p
    ~id:2 ~side:Sell ~instrument:inst
    ~quantity:(d 5.0) ~price:(d 100.0)
    ~slippage_buffer:0.0 ~fee_rate:0.0
  in
  Alcotest.(check (float 1e-6)) "cash unchanged"
    8000.0 (Decimal.to_float p.cash);
  Alcotest.(check (float 1e-6)) "available_cash unchanged for sells"
    8000.0 (Decimal.to_float (Engine.Portfolio.available_cash p));
  Alcotest.(check (float 1e-6)) "available_qty dropped"
    15.0 (Decimal.to_float (Engine.Portfolio.available_qty p inst))

let test_commit_fill_removes_reservation () =
  let p = Engine.Portfolio.empty ~cash:(d 10_000.0) in
  let p = Engine.Portfolio.reserve p
    ~id:3 ~side:Buy ~instrument:inst
    ~quantity:(d 10.0) ~price:(d 100.0)
    ~slippage_buffer:0.01 ~fee_rate:0.001
  in
  (* Commit with actual numbers slightly different from reservation. *)
  let p = Engine.Portfolio.commit_fill p
    ~id:3 ~actual_quantity:(d 10.0)
    ~actual_price:(d 99.5)
    ~actual_fee:(d 0.995)
  in
  (* Reservation gone, available_cash = cash = 10000 - 995 - 0.995 *)
  Alcotest.(check (float 1e-3)) "cash debited at actual"
    9004.005 (Decimal.to_float p.cash);
  Alcotest.(check (float 1e-3)) "available = cash again"
    (Decimal.to_float p.cash)
    (Decimal.to_float (Engine.Portfolio.available_cash p));
  Alcotest.(check int) "no reservations left" 0
    (List.length p.reservations)

let test_release_removes_reservation_without_touching_cash () =
  let p = Engine.Portfolio.empty ~cash:(d 10_000.0) in
  let p = Engine.Portfolio.reserve p
    ~id:4 ~side:Buy ~instrument:inst
    ~quantity:(d 10.0) ~price:(d 100.0)
    ~slippage_buffer:0.01 ~fee_rate:0.001
  in
  let p = Engine.Portfolio.release p ~id:4 in
  Alcotest.(check (float 1e-6)) "cash untouched"
    10_000.0 (Decimal.to_float p.cash);
  Alcotest.(check int) "reservation gone" 0
    (List.length p.reservations);
  Alcotest.(check (float 1e-6)) "available = cash"
    10_000.0 (Decimal.to_float (Engine.Portfolio.available_cash p))

let test_commit_unknown_id_raises () =
  let p = Engine.Portfolio.empty ~cash:(d 1000.0) in
  Alcotest.check_raises "commit unknown id"
    Not_found (fun () ->
      let _ = Engine.Portfolio.commit_fill p
        ~id:42 ~actual_quantity:(d 1.0)
...
let test_multiple_reservations_stack () =
  let p = Engine.Portfolio.empty ~cash:(d 10_000.0) in
  let p = Engine.Portfolio.reserve p
    ~id:5 ~side:Buy ~instrument:inst
    ~quantity:(d 10.0) ~price:(d 100.0)
    ~slippage_buffer:0.0 ~fee_rate:0.0 in
  let p = Engine.Portfolio.reserve p
    ~id:6 ~side:Buy ~instrument:inst
    ~quantity:(d 20.0) ~price:(d 100.0)
    ~slippage_buffer:0.0 ~fee_rate:0.0 in
  (* total reserved: 1000 + 2000 = 3000 *)
  Alcotest.(check (float 1e-6)) "available reduced by both"
    7000.0 (Decimal.to_float (Engine.Portfolio.available_cash p))
