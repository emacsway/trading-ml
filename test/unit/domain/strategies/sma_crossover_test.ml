...
let test_flat_gives_hold () =
  ...
  Alcotest.(check bool) "no enters on flat input" true
    (not (contains Signal.Enter_long acts));
  Alcotest.(check bool) "no exits either" true
...
let test_uptrend_triggers_long_entry () =
  ...
  Alcotest.(check bool) "enter_long fires on sustained uptrend" true
    (contains Signal.Enter_long acts)

let test_reversal_exits_long () =
  let strat = build ~fast:5 ~slow:10 ~allow_short:false in
  let acts = actions_from_prices strat rising_then_falling in
  Alcotest.(check bool) "enter_long fires" true
    (contains Signal.Enter_long acts);
  Alcotest.(check bool) "exit_long fires after reversal" true
...
let test_reversal_flips_when_short_allowed () =
  ...
  let acts = actions_from_prices strat rising_then_falling in
  Alcotest.(check bool) "enter_short replaces exit_long" true
    (contains Signal.Enter_short acts);
  Alcotest.(check bool) "no plain exit_long when flipping" true
    (not (contains Signal.Exit_long acts))
...
let test_enter_long_fires_once_per_cross () =
  ...
  Alcotest.(check int) "exactly one enter_long in a single trend"
    1 (List.length enters)

let test_rejects_fast_ge_slow () =
  Alcotest.check_raises "fast=slow invalid" (Invalid_argument "SMA_Crossover: fast < slow")
    (fun () -> ignore (build ~fast:10 ~slow:10 ~allow_short:false));
  Alcotest.check_raises "fast>slow invalid" (Invalid_argument "SMA_Crossover: fast < slow")
    (fun () -> ignore (build ~fast:20 ~slow:10 ~allow_short:false))
