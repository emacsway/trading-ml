...
let flat_then_spike_up =
  List.init 25 (fun _ -> 100.) @ [ 150. ]

let flat_then_spike_down =
  List.init 25 (fun _ -> 100.) @ [ 50. ]
...
let test_spike_above_band_enters_long () =
  ...
  Alcotest.(check bool) "enter_long on spike above upper band" true
...
let test_spike_below_band_enters_short () =
  ...
  Alcotest.(check bool) "enter_short on spike below lower band" true
...
let test_reversion_to_middle_exits () =
  ...
  Alcotest.(check bool) "enter_long fires" true
    (contains Signal.Enter_long acts);
  Alcotest.(check bool) "exit_long fires when close reverts below middle"
...
let test_flat_input_silent () =
  ...
  Alcotest.(check bool) "no entries on constant prices" true
    (not (contains Signal.Enter_long acts));
  Alcotest.(check bool) "no shorts either" true
