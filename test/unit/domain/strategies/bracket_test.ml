...
let build_bracket
    ...
    ?(atr_period = 14)
    inner =
  let params = Strategies.Bracket.{
    tp_mult; sl_mult; max_hold_bars; atr_period; inner;
  } in
...
let warmup_bars ~n =
  List.init n (fun i ->
    ohlc_bar ~ts:i ~close:100.0 ~high:100.3 ~low:99.7)
...
let test_timeout_exit () =
  ...
  let strat = build_bracket
    ~max_hold_bars:5
    (mk_stub Signal.Enter_long 0.9) in
  let post_entry = List.init 10 (fun i ->
    ohlc_bar ~ts:(16 + i) ~close:100.0 ~high:100.05 ~low:99.95) in
  let candles =
    warmup_bars ~n:15
    @ [ ohlc_bar ~ts:15 ~close:100.0 ~high:100.3 ~low:99.7 ]
    @ post_entry
...
let test_priority_ignores_inner_while_in_position () =
  ...
  in
  let reasons = actions_with_reasons strat candles in
  let enters = List.filter (fun (a, _) ->
    a = Signal.Enter_long) reasons in
  let exits = List.filter (fun (a, _) ->
    a = Signal.Exit_long) reasons in
...
let test_atr_warmup_swallows_entry () =
  ...
  let enters = List.filter (fun (a, _) ->
    a = Signal.Enter_long) reasons in
  Alcotest.(check int) "no entry fired during ATR warmup" 0
    (List.length enters)
...
let test_flat_inner_hold_passthrough () =
  ...
  let sigs = signals_from strat candles in
  let non_holds = List.filter (fun (s : Signal.t) ->
    s.action <> Signal.Hold) sigs in
  Alcotest.(check int) "only Hold emitted when inner is Hold" 0
    (List.length non_holds)
...
let tests = [
  ...
  "bracket ignores inner in position", `Quick, test_priority_ignores_inner_while_in_position;
