...
let build ?(period = 14) ?(lower = 30.) ?(upper = 70.)
          ?(exit_long = 50.) ?(exit_short = 50.)
          ?(allow_short = false) () =
  let p = Strategies.Rsi_mean_reversion.{
...
let test_oversold_enters_long () =
  ...
  Alcotest.(check bool) "enter_long fires on oversold" true
...
let test_overbought_shorts_when_allowed () =
  ...
  let prices = List.init 40 (fun i -> 100. +. float_of_int i) in
  let acts = actions_from_prices strat prices in
  Alcotest.(check bool) "enter_short on overbought uptrend" true
