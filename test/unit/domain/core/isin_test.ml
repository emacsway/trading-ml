...
let test_valid_known () =
  Alcotest.(check string) "SBER round-trips"
    sber_isin (Isin.to_string (Isin.of_string sber_isin));
  Alcotest.(check string) "AAPL round-trips"
    aapl_isin (Isin.to_string (Isin.of_string aapl_isin))

let test_normalises () =
  Alcotest.(check string) "lowercases accepted, upper-cased back"
    sber_isin (Isin.to_string (Isin.of_string (String.lowercase_ascii sber_isin)))

let test_rejects_wrong_length () =
  Alcotest.check_raises "11 chars"
    (Invalid_argument "Isin.of_string: \"RU000902954\" — expected 12 chars")
    (fun () -> ignore (Isin.of_string "RU000902954"))
...
let test_rejects_non_alnum () =
  Alcotest.check_raises "punctuation"
    (Invalid_argument "Isin.of_string: \"RU000902-540\" — non-alphanumeric")
    (fun () -> ignore (Isin.of_string "RU000902-540"))
