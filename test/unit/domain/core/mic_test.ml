...
let test_normalises () =
  Alcotest.(check string) "trims + uppercases"
    "XNYS" (Mic.to_string (Mic.of_string "  xnys  "));
  Alcotest.(check string) "uppercases mixed"
    "MISX" (Mic.to_string (Mic.of_string "MiSx"))

let test_rejects_wrong_length () =
  Alcotest.check_raises "too short"
    (Invalid_argument "Mic.of_string: \"ABC\" — expected 4 chars")
    (fun () -> ignore (Mic.of_string "ABC"));
  Alcotest.check_raises "too long"
    (Invalid_argument "Mic.of_string: \"ABCDE\" — expected 4 chars")
    (fun () -> ignore (Mic.of_string "ABCDE"))

let test_rejects_non_alnum () =
  Alcotest.check_raises "punctuation"
    (Invalid_argument "Mic.of_string: \"AB-D\" — non-alphanumeric")
    (fun () -> ignore (Mic.of_string "AB-D"))
