...
let test_shape () =
  Alcotest.(check int) "4 children → 10 features"
    10 (Logistic_regression.Features.n_features ~n_children:4);
  Alcotest.(check int) "2 children → 6 features"
    6 (Logistic_regression.Features.n_features ~n_children:2);
  Alcotest.(check int) "0 children → 2 features"
    2 (Logistic_regression.Features.n_features ~n_children:0)

let inst = Instrument.make
  ~ticker:(Ticker.of_string "X") ~venue:(Mic.of_string "MISX") ()
...
let test_extraction () =
  ...
  Alcotest.(check int) "length matches n_features"
...
let test_short_signal () =
  ...
  let candle = Candle.make ~ts:0L
    ~open_:(Decimal.of_float 100.0) ~high:(Decimal.of_float 100.0)
    ~low:(Decimal.of_float 100.0) ~close:(Decimal.of_float 100.0)
    ~volume:(Decimal.of_float 500.0) in
  let f = Logistic_regression.Features.extract
