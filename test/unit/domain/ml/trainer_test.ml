...
let test_smoke () =
  ...
  let result = Logistic_regression.Trainer.train
    ~children:(children ()) ~candles:(mk_candles prices)
    ~lookahead:5 ~epochs:5 () in
  ...
  Alcotest.(check bool) "val_loss finite"
    true (Float.is_finite result.val_loss);
  Alcotest.(check bool) "has training samples"
    true (result.n_train > 0);
  Alcotest.(check bool) "has validation samples"
    true (result.n_val > 0)
...
let test_too_few_bars () =
  ...
  let result = Logistic_regression.Trainer.train
    ~children:(children ()) ~candles:(mk_candles prices)
    ~lookahead:3 ~epochs:5 () in
  Alcotest.(check bool) "too few bars → no training"
    true (result.n_train = 0)
...
let test_train_val_split () =
  ...
  let result = Logistic_regression.Trainer.train
    ~children:(children ()) ~candles:(mk_candles prices)
    ~lookahead:5 ~epochs:5 () in
  Alcotest.(check bool) "train > val (70/30 split)"
    true (result.n_train > result.n_val)
