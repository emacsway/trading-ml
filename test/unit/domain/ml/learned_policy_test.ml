...
let prices = List.init 300 (fun i ->
  50.0 +. 15.0 *. sin (float_of_int i /. 8.0))
...
let test_learned_produces_signals () =
  ...
  Alcotest.(check bool) "produces some signals"
    true (n_hold < n_total);
  Alcotest.(check bool) "doesn't signal every bar"
    true (n_hold > 0)

let test_mock_predictor () =
  let always_long : Strategies.Composite.predictor =
    fun ~signals:_ ~candle:_ ~recent_closes:_ ~recent_volumes:_ -> 0.9 in
  let strat = Strategies.Strategy.make (module Strategies.Composite)
    Strategies.Composite.{
      policy = Learned { predict = always_long; threshold = 0.6 };
      children = children_build ();
    } in
  let acts = actions_from_prices strat
    (List.init 10 (fun _ -> 100.0)) in
  Alcotest.(check bool) "mock predictor → all Enter_long" true
