...
let test_json_round_trip () =
  ...
  Alcotest.(check (float 1e-12)) "round-trip predicts identically"
...
let test_json_missing_hyperparams_use_defaults () =
  ...
  let m = Logistic_regression.Logistic.of_json j in
  Alcotest.(check int) "3 weights" 3
...
let test_json_rejects_malformed () =
  ...
  Alcotest.check_raises "non-array weights → Invalid_argument"
    (Invalid_argument
       "Logistic.of_json: missing or non-array [weights]")
    (fun () -> ignore (Logistic_regression.Logistic.of_json bad))

let test_file_round_trip () =
  let path =
    Filename.concat
      (try Sys.getenv "TMPDIR" with Not_found -> "/tmp")
      (Printf.sprintf "logistic_test_%d_%d.json"
         (Unix.getpid ())
  ...
  Fun.protect
    ...
    (fun () ->
      ...
      let _ = Logistic_regression.Logistic.train m ~epochs:10
        [ [| 1.0; 0.0 |], 1.0; [| 0.0; 1.0 |], 0.0 ] in
      ...
      Alcotest.(check (float 1e-12)) "file round-trip identical"
        (Logistic_regression.Logistic.predict m probe)
        (Logistic_regression.Logistic.predict m' probe))
