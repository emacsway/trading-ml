...
let test_immediate_open_projects_working_lifecycle () =
  ...
  Alcotest.(check string) "directive.kind = IMMEDIATE" "IMMEDIATE"
    vm.directive.kind;
  Alcotest.(check (option string)) "no params for IMMEDIATE" None
    vm.directive.params;
  Alcotest.(check string) "lifecycle = WORKING" "WORKING" vm.lifecycle;
  Alcotest.(check (option string)) "no lifecycle_reason in WORKING" None
    vm.lifecycle_reason;
  Alcotest.(check string) "strategy.kind = IMMEDIATE" "IMMEDIATE"
    vm.strategy.kind;
  Alcotest.(check int) "one placement materialised at open" 1
    (List.length vm.placements)
...
let test_twap_directive_carries_params_blob () =
  ...
  Alcotest.(check int) "window_seconds in params blob" 60
...
let test_round_trip_json () =
  ...
  Alcotest.(check int) "placements count survives" (List.length vm.placements)
    (List.length vm'.placements)
