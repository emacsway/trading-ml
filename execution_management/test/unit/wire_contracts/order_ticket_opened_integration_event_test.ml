...
let test_opened_carries_correlation_intent_directive () =
  ...
  in
  let ie =
    Ie.Order_ticket_opened_integration_event.of_domain
      ~correlation_id:"saga-A" domain_ev
  in
  ...
  Alcotest.(check string) "directive.kind = IMMEDIATE" "IMMEDIATE"
    ie.directive.kind
...
let test_completed_carries_progress () =
  ...
  let progress =
    Values.Progress.apply_fill progress
      ~fill:
        (Ot.Placement.Values.Fill_record.make ~quantity:(qty "100")
  ...
  in
  let ie =
    Ie.Order_ticket_completed_integration_event.of_domain
      ~correlation_id:"saga-A" domain_ev
  in
  Alcotest.(check string) "total_quantity = 100" "100"
    ie.progress.total_quantity;
  Alcotest.(check string) "cumulative_filled = 100" "100"
    ie.progress.cumulative_filled;
...
let test_cancelled_carries_reason_string () =
  ...
  in
  let ie =
    Ie.Order_ticket_cancelled_integration_event.of_domain
      ~correlation_id:"saga-A" domain_ev
  in
  Alcotest.(check string) "reason serialised to wire form" "operator"
    ie.reason
...
let test_failed_carries_free_form_reason () =
  ...
  in
  let ie =
    Ie.Order_ticket_failed_integration_event.of_domain
      ~correlation_id:"saga-A" domain_ev
  in
  Alcotest.(check string) "reason propagated verbatim"
    "venue rejected: instrument suspended" ie.reason
