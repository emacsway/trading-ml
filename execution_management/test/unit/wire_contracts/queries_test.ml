...
let store_module =
  (module Store : Ports.Ticket_store.S with type t = Store.t)
...
let test_get_returns_none_on_miss () =
  ...
  Alcotest.(check bool) "miss is None" true
    (Option.is_none
       (Q.Get_order_ticket_query_handler.handle store_module
          ~store_handle:store q))

let test_get_returns_none_on_malformed_id () =
  let store = open_ticket_and_store 7 in
  let q : Q.Get_order_ticket_query.t = { ticket_id = 0 } in
  Alcotest.(check bool) "non-positive id rejected to None" true
    (Option.is_none
       (Q.Get_order_ticket_query_handler.handle store_module
          ~store_handle:store q))
...
let test_list_open_returns_all_non_terminal () =
  ...
  let result =
    Q.List_open_order_tickets_query_handler.handle store_module
      ~store_handle:store q
  in
