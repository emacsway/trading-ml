...
let test_init_emits_first_visible_chunk () =
  ...
  Alcotest.(check string) "first chunk = visible_qty" "10"
...
let test_total_smaller_than_visible_emits_total () =
  ...
  let params = Values.Iceberg_params.make ~visible_qty:(qty "10") in
  let _state, decision = Iceberg.init ~intent ~params ~now:0L in
  Alcotest.(check string) "chunk = total when remaining < visible" "5"
    (Decimal.to_string (List.hd decision.submit).quantity)
  ...
  let _state', decision =
    Iceberg.on_event state
      (Input.Placement_filled
         { placement_id = placement_id_1; fill = full_fill "10" })
      ~now:1L
  in
  Alcotest.(check int) "next chunk emitted" 1 (List.length decision.submit);
  Alcotest.(check string) "next chunk = visible_qty" "10"
    (Decimal.to_string (List.hd decision.submit).quantity)
...
let test_partial_fill_holds_next_chunk () =
  ...
  let _state', decision =
    Iceberg.on_event state
      (Input.Placement_filled
         { placement_id = placement_id_1; fill = full_fill "4" })
      ~now:1L
  in
  Alcotest.(check int) "no refill on partial fill" 0
...
let test_two_partial_fills_complete_chunk () =
  ...
  let _state', decision =
    Iceberg.on_event state
      (Input.Placement_filled
         { placement_id = placement_id_1; fill = full_fill "6" })
      ~now:2L
  in
  Alcotest.(check int) "next chunk emitted after chunk complete" 1
    (List.length decision.submit)
...
let test_full_intent_via_repeated_refills () =
  ...
  Alcotest.(check bool) "complete after Σ fills = total" true
...
let test_last_chunk_smaller_than_visible () =
  ...
  let _state', decision =
    Iceberg.on_event state
      ...
      ~now:2L
  in
  Alcotest.(check string) "last chunk = remaining (5)" "5"
    (Decimal.to_string (List.hd decision.submit).quantity)
  ...
  let _state', decision =
    Iceberg.on_event state
      (Input.Placement_rejected
         { placement_id = placement_id_1; reason = "venue down" })
      ~now:1L
  in
