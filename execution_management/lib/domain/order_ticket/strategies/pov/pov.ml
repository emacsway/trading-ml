...
let init ~(intent : Values.Trade_intent.t) ~(params : Values.Pov_params.t)
    ~now:_ =
  let state =
    {
...
let on_event (state : state) (input : Input.t) ~now:_ : state * Decision.t =
  if state.failed then (state, Decision.empty)
    ...
    | Input.Volume_bar { bar } ->
        ...
        let target =
          Decimal.of_float
            (Decimal.to_float observed' *. state.participation_rate)
        in
        ...
        if Decimal.is_positive emit_qty then
          ...
          in
          ( state',
            { Decision.empty with submit = [ market_submit emit_qty ] } )
        else
          ({ state with observed_volume = observed' }, Decision.empty)
    ...
    | Input.Tick _ | Input.Price_quote _
    | Input.Placement_acknowledged _ | Input.Placement_filled _ ->
        (state, Decision.empty)

let is_complete state =
  (not state.failed)
  && Decimal.compare (remaining state) Decimal.zero <= 0
