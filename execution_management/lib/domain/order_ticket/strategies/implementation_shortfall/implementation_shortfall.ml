...
let build_trajectory ~(total : Decimal.t)
  ...
  let kappa =
    sqrt (params.risk_aversion *. (params.volatility ** 2.0)
          /. params.temp_impact_eta)
  in
  ...
  let remaining_fraction t =
    if denom = 0.0 || not (Float.is_finite denom) then
      1.0 -. (t /. window)
    else
      sinh (kappa *. (window -. t)) /. denom
  in
  ...
  for i = 0 to n - 2 do
    ...
    in
    let due_at =
      Int64.add params.start_at (Int64.of_int (int_of_float t_next))
    in
    trajectory.(i) <- { due_at; quantity = slice_q };
    acc_first_n_minus_1 := Decimal.add !acc_first_n_minus_1 slice_q
  done;
  let last_due_at =
    Int64.add params.start_at (Int64.of_int params.window_seconds)
  in
  trajectory.(n - 1) <-
...
let init ~(intent : Values.Trade_intent.t)
    ~(params : Values.Implementation_shortfall_params.t) ~now:_ =
    ...
    | Input.Tick { now } ->
        if state.emitted_count >= n then (state, Decision.empty)
          ...
          if Int64.compare now next.due_at < 0 then (state, Decision.empty)
            ...
            let state' =
              { state with emitted_count = state.emitted_count + 1 }
            in
            ( state',
              { Decision.empty with submit = [ market_submit next.quantity ] } )
