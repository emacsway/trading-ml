...
let make ~n_slices ~window_seconds ~start_at ~volume_profile =
  ...
  if window_seconds <= 0 then
    invalid_arg "Vwap_params.make: window_seconds must be > 0";
  if List.length volume_profile <> n_slices then
    invalid_arg
      "Vwap_params.make: volume_profile length must equal n_slices";
  ...
  if sum <= 0.0 then
    invalid_arg "Vwap_params.make: weights must sum to a positive value";
