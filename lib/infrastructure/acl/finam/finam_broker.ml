...
let make ~account_id (rest : Rest.t) : t = {
  rest; account_id;
  srv_by_cid = Hashtbl.create 16;
  mutex = Eio.Mutex.create ();
}

let bars t ~n ~instrument ~timeframe =
  Rest.bars t.rest ~n ~instrument ~timeframe
...
let resolve_server_id t ~client_order_id =
  let cached = Eio.Mutex.use_ro t.mutex (fun () ->
    Hashtbl.find_opt t.srv_by_cid client_order_id)
  in
  match cached with
  | Some id -> id
  | None ->
    ...
    match List.find_opt
            (fun o -> o.Order.client_order_id = client_order_id) orders
    with
    | Some o ->
    ...
    | None ->
      failwith
        (Printf.sprintf "finam: no order with client_order_id=%s"
           client_order_id)

let place_order t ~instrument ~side ~quantity ~kind ~tif ~client_order_id =
  let o = Rest.place_order t.rest
    ~account_id:t.account_id
    ~instrument ~side ~quantity ~kind ~tif ~client_order_id () in
  remember t ~client_order_id ~server_id:o.id;
  o

let get_orders t =
  Rest.get_orders t.rest ~account_id:t.account_id
