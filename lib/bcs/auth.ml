(** BCS OAuth2 / Keycloak auth.
    Flow:
      1. Holder has a long-lived *refresh_token* (from the BCS portal).
      2. [current] POSTs [token_endpoint] with
           grant_type=refresh_token&refresh_token=…&client_id=…
         in [application/x-www-form-urlencoded] and parses the
         access_token + expires_in from the JSON response.
      3. The access_token goes into [Authorization: Bearer …]; it's
         cached until [expires_in - margin] seconds.
...
let http_refresh t : jwt =
  let body = urlencode_form [
    ...
    "refresh_token", t.cfg.Config.refresh_token;
  ...
  if resp.status < 200 || resp.status >= 300 then
    failwith (Printf.sprintf
      "BCS Auth: token endpoint returned %d: %s" resp.status resp.body);
  let j = Yojson.Safe.from_string resp.body in
  let open Yojson.Safe.Util in
  let token = match member "access_token" j with
    | `String s -> s
    | _ -> failwith ("BCS Auth: no access_token in " ^ resp.body)
  in
  let expires_in = match member "expires_in" j with
