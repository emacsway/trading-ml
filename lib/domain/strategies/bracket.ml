...
type position =
  | Flat
  | Long of bracket_state
  | Short of bracket_state
...
let init p =
  if p.tp_mult <= 0.0 then
    invalid_arg "Bracket: tp_mult > 0";
  if p.sl_mult <= 0.0 then
    invalid_arg "Bracket: sl_mult > 0";
  if p.max_hold_bars <= 0 then
    invalid_arg "Bracket: max_hold_bars > 0";
  if p.atr_period <= 1 then
    invalid_arg "Bracket: atr_period > 1";
  {
    params = p;
...
let check_long_exit ~high ~low bs =
  if low <= bs.sl then Some "SL hit"
  else if high >= bs.tp then Some "TP hit"
  else None

let check_short_exit ~high ~low bs =
  if high >= bs.sl then Some "SL hit"
  else if low <= bs.tp then Some "TP hit"
  else None
...
let on_candle st instrument (c : Candle.t) =
  ...
  | Long bs ->
    ...
    let exit_reason =
      ...
      | Some r -> Some r
      | None ->
        if bars_held >= st.params.max_hold_bars then Some "timeout"
        else None
    in
    (match exit_reason with
     | Some reason ->
       let sig_ = {
         ...
         action = Signal.Exit_long; strength = 1.0;
  ...
  | Short bs ->
    ...
    (match exit_reason with
     | Some reason ->
       let sig_ = {
         Signal.ts = c.Candle.ts; instrument;
         action = Signal.Exit_short; strength = 1.0;
         stop_loss = None; take_profit = None; reason;
       } in
       { st with position = Flat }, sig_
     | None ->
