...
let train
  ...
  let closes = Array.of_list
    (List.map (fun (c : Candle.t) -> Decimal.to_float c.close) candles) in
  let volumes = Array.of_list
    (List.map (fun (c : Candle.t) -> Decimal.to_float c.volume) candles) in
  ...
  for i = 0 to n_bars - lookahead - 1 do
    ...
    let has_signal =
      List.exists (fun (s : Signal.t) -> s.action <> Signal.Hold) signals in
    if has_signal then begin
      let recent_closes =
        let start = max 0 (i - context_window + 1) in
        Array.to_list (Array.sub closes start (i - start + 1))
        |> List.rev
      in
      let recent_volumes =
        let start = max 0 (i - context_window + 1) in
        Array.to_list (Array.sub volumes start (i - start + 1))
        |> List.rev
      in
      ...
      let target =
        if future_close > current_close then 1.0 else 0.0 in
  ...
  if n_total < 10 then
    ...
    let val_loss =
      if val_data = [] then 0.0
        ...
        let total = List.fold_left (fun acc (features, target) ->
