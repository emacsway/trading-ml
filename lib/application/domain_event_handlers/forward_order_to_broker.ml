...
let forward_rejection_to_string = function
  | Order_rejected_by_broker { reason; _ } ->
    Printf.sprintf "broker rejected: %s" reason
  | Broker_unreachable { reason; _ } ->
    Printf.sprintf "broker unreachable: %s" reason
...
let handle
    ...
    ~(tif : Order.time_in_force)
    ~(client_order_id : string)
    (ev : Engine.Portfolio.amount_reserved)
    : (order_forwarded, forward_rejection) Rop.t =
  try
    let broker_order = place_order
    ...
    | Rejected ->
      Rop.fail (Order_rejected_by_broker {
        client_order_id;
