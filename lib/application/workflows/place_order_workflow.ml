...
let run
    ...
    ~(place_order :
        Domain_event_handlers.Forward_order_to_broker.place_order_port)
  ...
  let* (portfolio', reserved) =
    Commands.Place_order_command.reserve
      ~portfolio ~market_price ~slippage_buffer ~fee_rate
      ~next_reservation_id u
  ...
  match
    Domain_event_handlers.Forward_order_to_broker.handle
      ~place_order
      ~kind:u.kind ~tif:u.tif ~client_order_id:u.client_order_id
      reserved
  with
  | Ok forwarded ->
    Ok (portfolio', events_so_far @ [Order_forwarded forwarded])
  | Error rejs ->
    let rejection = first rejs in
    let events_with_rejection =
      events_so_far @ [Forward_rejected rejection]
    in
    ...
    (match
     ...
     | Ok (portfolio'', released) ->
       Ok (portfolio'',
           events_with_rejection @ [Reservation_released released])
     | Error _ ->
       Ok (portfolio', events_with_rejection))
