...
let update_drawdown t (event : Engine.Pipeline.event) =
  if t.cfg.max_drawdown_pct > 0.0 && not t.halted then begin
    ...
    if Decimal.compare equity t.peak_equity > 0 then
    ...
    if peak > 0.0 then begin
      ...
      if drawdown > t.cfg.max_drawdown_pct then begin
        t.halted <- true;
        Log.warn "[engine] kill switch tripped: drawdown=%.2f%% \
                  (peak=%s, current=%s) — halted"
          (drawdown *. 100.0)
...
let on_fill_event t (fe : fill_event) =
  with_lock t (fun () ->
    ...
    | Some p ->
      let new_remaining =
        Decimal.sub p.remaining_quantity fe.actual_quantity in
      if Decimal.compare new_remaining Decimal.zero <= 0 then begin
        ...
        Log.info "[engine] commit (full) cid=%s qty=%s @ %s"
          fe.client_order_id
        ...
        t.state <- Engine.Step.commit_partial_fill t.state
          ...
          ~actual_price:fe.actual_price
          ~actual_fee:fe.actual_fee;
        Log.info "[engine] commit (partial) cid=%s qty=%s @ %s, \
                  %s remaining"
          fe.client_order_id
          (Decimal.to_string fe.actual_quantity)
          (Decimal.to_string fe.actual_price)
...
let reset t =
  with_lock t (fun () ->
    ...
    t.halted <- false;
    Log.info "[engine] reset — peak_equity=%s"
      (Decimal.to_string t.peak_equity))
