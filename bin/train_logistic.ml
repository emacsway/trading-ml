...
let build_children (spec : string) : Strategies.Strategy.t list =
  ...
  |> List.map (fun name ->
    ...
    | None ->
      Printf.eprintf
        "train-logistic: unknown strategy %S (run `trading list` for the \
         registered set)\n" name;
      exit 2)
...
let () =
  ...
  in
  let lookahead = match arg_value "--lookahead" args with
    | Some v -> int_of_string v | None -> 5 in
  let epochs = match arg_value "--epochs" args with
    | Some v -> int_of_string v | None -> 10 in
  let lr = match arg_value "--lr" args with
    | Some v -> float_of_string v | None -> 0.01 in
  let l2 = match arg_value "--l2" args with
    | Some v -> float_of_string v | None -> 1e-4 in
  let context_window = match arg_value "--context-window" args with
    | Some v -> int_of_string v | None -> 20 in
  ...
  let child_names =
    List.map Strategies.Strategy.name children |> String.concat ", " in
  Printf.printf "Children (%d): %s\n%!" (List.length children) child_names;

  let result = Logistic_regression.Trainer.train
    ~children ~candles ~lookahead ~epochs ~lr ~l2 ~context_window () in
  Printf.printf
    "Trained: n_train=%d n_val=%d train_loss=%.4f val_loss=%.4f\n%!"
    result.n_train result.n_val
    result.train_loss result.val_loss;

  if result.n_train = 0 then begin
    prerr_endline
      "No training samples collected. Causes: too few candles, \
       children always Hold, or lookahead too large for the history.";
    exit 3
  end;
  ...
  let model = Logistic_regression.Logistic.of_weights
    ~lr ~l2 result.weights in
